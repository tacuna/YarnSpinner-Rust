...
fn read_resource(_: In<()>, previously_added_resource: Res<SomethingAddedByYarnSpinner>) {
    println!("{} is {} years old", previously_added_resource.name, previously_added_resource.age);
}
...
fn update_resource(In((name, age)): In<(String, Option<f32>)>, mut resource: ResMut<SomethingAddedByYarnSpinner>) {
    resource.name = name;
    if let Some(age) = age {
        resource.age = age;
    }
