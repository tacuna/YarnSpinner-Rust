...
impl fmt::Display for MarkupParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            UnmatchedCloseMarker { input, name, position } => write!(f, "Unexpected close marker {name} at position {position} in line {input}"),
            ...
            UnterminatedMarker { input, name, position } => write!(f, "Unterminated marker {name} in line {input} at position {position}"),
        }
    }
}
