...
pub(crate) trait CommonTokenStreamExt<'input> {
    ...
    fn get_hidden_tokens_to_left(&self, token_index: isize, channel: i32) -> Vec<CommonToken<'input>>;

    /// Collect all tokens on specified channel to the right of
    /// the current token up until we see a token on
    /// [`Lexer::DefaultTokenChannel`] or EOF.
    /// If `channel` is `-1`, find any non default channel token.
    fn get_hidden_tokens_to_right(&self, token_index: isize, channel: i32) -> Vec<CommonToken<'input>>;
...
impl<'input> CommonTokenStreamExt<'input> for ActualTokenStream<'input> {
    ...
    fn get_hidden_tokens_to_right(&self, token_index: isize, channel: i32) -> Vec<CommonToken<'input>> {
        ...
        let to = if next_on_channel != -1 { next_on_channel } else { self.size() - 1 };
...
pub(crate) trait YarnSpinnerParserContextExt<'input>: YarnSpinnerParserContext<'input> {
    /// Same as [`YarnSpinnerParserContext::child_of_type`], but without the [`Sized`] requirement.
    fn child_of_type_unsized<T: YarnSpinnerParserContext<'input>>(&self, pos: usize) -> Option<Rc<T>> {
        self.get_children().filter_map(|child| child.downcast_rc::<T>().ok()).nth(pos)
    }
}
impl<'input, T: YarnSpinnerParserContext<'input> + ?Sized> YarnSpinnerParserContextExt<'input> for T {}

pub(crate) fn create_common_token<'a>(token_type: i32, text: impl Into<String>) -> Box<CommonToken<'a>> {
    ...
    CommonTokenFactory.create::<InputStream<&'a str>>(None, token_type, Some(text.into()), TOKEN_DEFAULT_CHANNEL, 0, 0, 0, -1)
}
