...
pub(crate) fn create_declarations_for_tracking_nodes(mut state: CompilationIntermediate) -> CompilationIntermediate {
    let tracking_declarations = state.tracking_nodes.iter().map(|node| {
        ...
        Declaration::new(name, Type::Number)
            ...
            .with_description(format!("The generated variable for tracking visits of node {node}"))
    });
    ...
    state.known_variable_declarations.extend(tracking_declarations.clone());
    state.derived_variable_declarations.extend(tracking_declarations);
